...
def genre_2d_pca_visualization(
    df: pd.DataFrame,
    features: list[str],
    genres: list[str],
    n_components: int = 2,
...
def genre_2d_umap_visualization(
    df: pd.DataFrame,
    features: list[str],
    genres: list[str],
    n_components: int = 2,
...
def genre_3d_umap_visualization(
    df: pd.DataFrame,
    label_column: str,
    n_components: int = 3,
    n_neighbors: int = 15,
...
def corr_heatmap(
    ...
    sns.heatmap(df.corr(method=method), cmap="RdBu", annot=True, vmin=-1, vmax=1)
...
def plot_outliers_per_genre(
    pca_df: pd.DataFrame, outliers_indexes: Union[pd.Index, list]
) -> None:
    """
    ...
    for genre in pca_df["genre"].unique():
        ...
        ax.scatter(
            df_genre.loc[outliers_indexes, "PC1"],
            df_genre.loc[outliers_indexes, "PC2"],
            color="purple",
            marker="+",
            s=30,
        )
...
def plot_num_outliers_per_genre(
    df: pd.DataFrame, outliers_indexes: Union[pd.Index, list]
) -> None:
    """
...
def plot_cluster_similarities(
    label: str, label_value: str, similarities_df: pd.DataFrame, ax: Axes
) -> None:
    """
    ...
    """
    similarities_df = similarities_df.query(
        f'{label}_1 == "{label_value}" or {label}_2 == "{label_value}"'
    ).sort_values(by="similarity", ascending=False)
    similarities_df[label] = similarities_df[f"{label}_1"].str.cat(
        similarities_df[f"{label}_2"], sep=" <-> "
    )
    ax.set_title(f"Genre similarity for {label_value}")
    sns.barplot(
        data=similarities_df, x="similarity", y=f"{label}", ax=ax, color="#00B3B3"
    )
