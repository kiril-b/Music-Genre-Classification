...
class ClassificationEvaluation():
    ...
    def __init__(self, dataset: Dataset, classifier: ClassifierMixin, encoder: ClassifierMixin = None):
        """
    ...
    def make_predictions(self) -> None:
        ...
        if self.label_encoder is not None and not isinstance(self.classifier.classes_[0], str):
    ...
    def get_scores(self, on_sets: list[str] = None, average_type: str = 'weighted') -> pd.DataFrame:
        """
        ...
        train_y, val_y, test_y = self.dataset.train.y, self.dataset.val.y, self.dataset.test.y

        for dataset in on_sets:
            if dataset == 'train':
                data.append([accuracy_score(train_y, self.train_y_pred),
                             precision_score(train_y, self.train_y_pred, average=average_type, zero_division=0),
                             recall_score(train_y, self.train_y_pred, average=average_type),
                             f1_score(train_y, self.train_y_pred, average=average_type)])
            elif dataset == 'val':
                data.append([accuracy_score(val_y, self.val_y_pred),
                             precision_score(val_y, self.val_y_pred, average=average_type, zero_division=0),
                             recall_score(val_y, self.val_y_pred, average=average_type),
                             f1_score(val_y, self.val_y_pred, average=average_type)])
            elif dataset == 'test':
                data.append([accuracy_score(test_y, self.test_y_pred),
                             precision_score(test_y, self.test_y_pred, average=average_type, zero_division=0),
                             recall_score(test_y, self.test_y_pred, average=average_type),
                             f1_score(test_y, self.test_y_pred, average=average_type)])
            else:
                raise ValueError("on_sets may only contain values from the following: {'train', 'val', 'test'}")
    ...
    def plot_confusion_matrix(self, ax: Axes, on_set: str) -> None:
        ...
        if on_set not in ['train', 'val', 'test']:
            raise ValueError("on_sets may only be one of the following: ['train', 'val', 'test']'")
        ...
        sns.heatmap(
            cm, annot=True, cmap=mcolors.LinearSegmentedColormap.from_list("custom_colormap", ['white', '#34d4a1']),
            vmin=0, vmax=1,
    ...
    def plot_metrics_per_class(self, ax: Axes, on_set: str, horizontal: bool = True) -> None:
        """
        ...
        sns.heatmap(results, vmin=0, vmax=1, cmap=mcolors.LinearSegmentedColormap.from_list("custom_colormap", ['white', '#34d4a1']), annot=True, ax=ax)
...
class DTClassificationEvaluation(ClassificationEvaluation):
    ...
    def __init__(self, dataset: Dataset, classifier: ClassifierMixin, encoder: ClassifierMixin):
        super().__init__(dataset, classifier, encoder)

    def plot_feature_importances(self, ax: Axes, num_features: int = 10, horizontal: bool = False) -> None:
        """
        ...
        if horizontal:
            sns.barplot(y=importances[indices][:num_features], x=names, ax=ax, color='coral')
        else:
            sns.barplot(x=importances[indices][:num_features], y=names, ax=ax, color='coral')
...
class Classifier:
    ...
    """
    def __init__(self, dataset: Dataset, classifier: ClassifierMixin, evaluation: ClassificationEvaluation):
...
class ClassifiersCollection:
    ...
    def evaluate_classifiers(self) -> pd.DataFrame:
        ...
        for classifier in self.classifiers:
            df_scores = classifier.get_clf_evaluation().get_scores(on_sets=['train', 'val', 'test'])
...
class ClassifierFactory:
    ...
    @staticmethod
    def create_instance(dataset: Dataset, classifier: ClassifierMixin, encoder: ClassifierMixin = None, tree_type_classifier: bool = False):
        """
