...
class DatasetsWrapper():
    ...
    def scale_datasets(self, scaler: TransformerMixin, exclude: list[str] = None) -> None:
        ...
        for name, dataset in self.data_dict.items():
            ...
            train.X = pd.DataFrame(scaler.fit_transform(train.X), index=train.X.index, columns=train.X.columns)
            val.X   = pd.DataFrame(scaler.transform(val.X), index=val.X.index, columns=val.X.columns)
            test.X  = pd.DataFrame(scaler.transform(test.X), index=test.X.index, columns=test.X.columns)
    ...
    def encode_labels(self, encoder: TransformerMixin) -> None:
        ...
        for _, dataset in self.data_dict.items():
            ...
            train.y = pd.Series(self.label_encoder.fit_transform(train.y), index=train.y.index)
    ...
    def reverse_encode_labels(self) -> None:
        ...
        if self.label_encoder is None:
            raise Exception('The lables were not encoded previously. Make sure to call "encode_labels" first')
        
        for _, dataset in self.data_dict.items():

            train, val, test = dataset.train, dataset.val, dataset.test

            train.y = pd.Series(self.label_encoder.inverse_transform(train.y), index=train.y.index)
            val.y   = pd.Series(self.label_encoder.inverse_transform(val.y), index=val.y.index)
            test.y  = pd.Series(self.label_encoder.inverse_transform(test.y), index=test.y.index)
    ...
    def get_shapes(self) -> pd.DataFrame:
        ...
        """
        shapes = [[
                dataset.train.X.shape, dataset.val.X.shape, dataset.test.X.shape,
                dataset.train.y.shape, dataset.val.y.shape, dataset.test.y.shape
            ] for dataset in self.data_dict.values()]
        index = self.data_dict.keys()

        return pd.DataFrame(shapes, index=index, columns=['train_X', 'val_X', 'test_X', 'train_y', 'val_y', 'test_y'])
...
def create_dataset(name: str, 
                   train_X: pd.DataFrame, train_y: Union[pd.DataFrame, pd.Series], 
                   val_X: pd.DataFrame, val_y: Union[pd.DataFrame, pd.Series], 
                   test_X: pd.DataFrame, test_y: Union[pd.DataFrame, pd.Series]
                   ) -> Dataset:
    """
...
def load_datasets() -> list[Dataset]:
    ...
    train_spectral_original_X = pd.read_csv(os.getenv('train_spectral_original_X'), index_col=0, header=[0, 1, 2])
    train_spectral_modified_X = pd.read_csv(os.getenv('train_spectral_modified_X'), index_col=0, header=[0, 1, 2])
    val_spectral_X = pd.read_csv(os.getenv('val_spectral_X'), index_col=0, header=[0, 1, 2])
    test_spectral_X = pd.read_csv(os.getenv('test_spectral_X'), index_col=0, header=[0, 1, 2])

    train_all_original_X = pd.read_csv(os.getenv('train_all_original_X'), index_col=0, header=[0, 1, 2])
    train_all_modified_X = pd.read_csv(os.getenv('train_all_modified_X'), index_col=0, header=[0, 1, 2])
    ...
    spectral_original_dataset = create_dataset("spectral_original", 
                                               train_spectral_original_X, train_original_y,
                                               val_spectral_X, val_y,
                                               test_spectral_X, test_y)


    all_original_dataset = create_dataset("all_original", 
                                               train_all_original_X, train_original_y,
                                               val_all_X, val_y,
                                               test_all_X, test_y)


    pca_original_dataset = create_dataset("pca_original", 
                                               train_original_pca_X, train_original_y,
                                               val_original_pca_X, val_y,
                                               test_original_pca_X, test_y)
    
    umap_original_dataset = create_dataset("umap_original", 
                                               train_original_umap_X, train_original_y,
                                               val_original_umap_X, val_y,
                                               test_original_umap_X, test_y)

    spectral_modified_dataset = create_dataset("spectral_modified", 
                                               train_spectral_modified_X, train_modified_y,
                                               val_spectral_X, val_y,
                                               test_spectral_X, test_y)


    all_modified_dataset = create_dataset("all_modified", 
                                               train_all_modified_X, train_modified_y,
                                               val_all_X, val_y,
                                               test_all_X, test_y)

    pca_modified_dataset = create_dataset("pca_modified", 
                                               train_modified_pca_X, train_modified_y,
                                               val_modified_pca_X, val_y,
                                               test_modified_pca_X, test_y)
    
    umap_modified_dataset = create_dataset("umap_modified", 
                                               train_modified_umap_X, train_modified_y,
                                               val_modified_umap_X, val_y,
                                               test_modified_umap_X, test_y)
